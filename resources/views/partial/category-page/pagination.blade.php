        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 text-left">
                <ul class="pagination ">
                    ...
                    @if($isPrev)
                        <li>
                            <a href="{{ url_category_per_page($model->currentCategory->name_slug . ($model->sort == 'price-asc' || $model->sort == 'price-desc' ? ('/' . $model->sort) : ''), $model->page - 1, $model->language) }}"
                    ...
                    @foreach($pages as $page)
                        <li class="{{ $page == $model->page ? 'current' : '' }}">
                            ...
                            @else
                                ...
                                @else
                                    <a href="{{ url_category_per_page($model->currentCategory->name_slug . ($model->sort == 'price-asc' || $model->sort == 'price-desc' ? ('/' . $model->sort) : ''), $page, $model->language) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endif
